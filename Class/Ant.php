...
class Ant {
	...
	//кількість мурах
	public $Pg;
	//коеф глобального випаровування
	public $Mu;
	//коеф локального випаровування
	public $CF = 0;
	//Цільова функція
	public $task;
	//задача
	public $alpha;
	//коеф жадібності мурах
	public $beta;
	//коеф стадності мурах
	public $pkm;
	...
	private function globalUpdate($start, $move) {
		if ($this -> pheromone[$start][$move] > 0.001 && !in_array($start, $this -> task -> tabuArray) && !in_array($move, $this -> task -> tabuArray)) {
			$this -> pheromone[$start][$move] = $this -> Pg * $this -> pheromone[$start][$move] + (1 - $this -> Pg) * $this -> sumAnt($start, $move);
	...
	private function addPheromone($start, $move) {
		if (!in_array($start, $this -> task -> tabuArray) && !in_array($move, $this -> task -> tabuArray)) {
			$this -> pheromone[$start][$move] = $this -> pheromone[$start][$move] + $this -> Mu * (1 - $this -> pheromone[$start][$move]);
			$this -> pheromone[$move][$start] = $this -> pheromone[$start][$move];
		}
	}
	...
	private function selectEdge() {
		...
		while ($i < $this -> task -> amountV) {
			...
			if ($this -> pheromone[$this -> start][$i] != TaskGenerator::$BigNum && $this -> pheromone[$this -> start][$i] != 0 && $this -> task -> Matrix[$this -> start][$i] != TaskGenerator::$BigNum && $this -> task -> Matrix[$this -> start][$i] != 0 && !in_array($i, $this -> tabuList)) {
				//чисельник
	...
	private function sumEdges() {
		for ($i = 0; $i < $this -> task -> amountV; $i++) {
			...
			if ($this -> task -> Matrix[$this -> start][$i] != TaskGenerator::$BigNum && $this -> task -> Matrix[$this -> start][$i] != 0 && !in_array($i, $this -> tabuList)) {
				//сумування
